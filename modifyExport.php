...
require_once ('dbhelp.php');
	...
	if (!empty($_POST)) {
		...
		if (empty($error)) {
			$sql = "update export set n_staff = '$s_sname', time = '$s_time', storage_address = '$s_address', n_book = '$s_bname', id_book = '$s_bid', amount_of_book = '$s_ammount', current_price_of_book = '$s_price', id_staff = '$s_sid' where id_note = " .$s_id;
			execute($sql);
	...
	if (isset($_GET['id'])) {
		$id          = $_GET['id'];
		$sql         = 'select * from export where id_note = '.$id;
...
<html>
<head>
	<title>Modify Export Note</title>
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h2 class="text-center">Chỉnh sửa dữ liệu xuất</h2>
			</div>
